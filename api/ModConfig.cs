        ...
        try
        {
            switch (Type)
            {
                ...
                case ConfigItemType.SELECT:
                    ...
                    IntVal = Convert.ToInt32(val);
                    IntVal = ModConfigSelectOptionCodec.ClampIndex(IntVal, ModConfigSelectOptionCodec.Parse(TextVal));
                    if (!string.IsNullOrEmpty(CallBack) && !pSkipCallback)
                    {
                        MethodInfo callback = AccessTools.Method(CallBack, new Type[1] { typeof(int) });
                        if (callback == null)
...
public partial class ModConfig
{
        ...
        switch (pItem.Type)
        {
            ...
            case ConfigItemType.SELECT:
                ...
                return new SelectSchemaEntry
                {
                    ...
                    CallBack = pItem.CallBack,
                    DefaultValue = pItem.IntVal,
                    OptionsRaw = options_raw,
                    Options = ParseSelectOptions(options_raw)
                };
            default:
    ...
    private static string[] ParseSelectOptions(string pRaw)
    {
        return ModConfigSelectOptionCodec.Parse(pRaw);
    }

    private static int ClampSelectIndex(int pIndex, string[] pOptions)
    {
        return ModConfigSelectOptionCodec.ClampIndex(pIndex, pOptions);
    }
    ...
    /// <param name="pOptions">Select options, persisted in legacy field <see cref="ModConfigItem.TextVal" /></param>
    /// <param name="pIconPath"></param>
    /// <param name="pCallback"></param>
    /// <param name="pSkipCallback">Whether skip callback while setting initial value</param>
    /// <returns></returns>
        ...
        string pIconPath = "", string pCallback = "", bool pSkipCallback = false)
    {
        ...
        item.CallBack = pCallback;
        string options_raw = ModConfigSelectOptionCodec.Serialize(pOptions);
        item.TextVal = options_raw;
        int normalized_index = ModConfigSelectOptionCodec.ClampIndex(pDefaultIndex, ModConfigSelectOptionCodec.Parse(options_raw));
